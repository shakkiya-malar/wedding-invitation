...
function submitWish() {
    ...
    .then(() => {
        ...
        // ✅ ADD IMMEDIATELY (no refresh)
addWishToUI({
    name,
    message,
    time: new Date().toISOString()
});
        // ✅ optional reload (sync with sheet)
...
function addWishToUI(wish) {
   ...
   div.innerHTML = `
    <div class="chat-bubble ${getBubbleSide(wish.name)}">
        
        <div class="bubble-content">
            <div class="bubble-name">${wish.name}</div>
            <div class="bubble-message">${wish.message}</div>

            <div class="bubble-meta">
                <span class="bubble-time">${formatTime(wish.time)}</span>
            </div>
        </div>
...
function getBubbleSide(name) {
    return name.length % 2 === 0 ? "right" : "left";
}
function formatTime(ts) {
    ...
    if (isNaN(time)) return "Just now"; // fallback
